<script lang="ts">
  import { isHotkeysMenuVisible, appConfig, type Shortcuts } from "$lib/store";
      ...
      {#if $appConfig.shortcuts}
        {#each Object.keys($appConfig.shortcuts) as action}
          ...
          <div class="hotkey-key">
            {$appConfig.shortcuts[action as keyof Shortcuts].label}
          </div>
        {/each}
      {/if}
    </div>
    <button on:click={handleClose} class="close-button">{$t["hotkeys.close"]}</button>
  </div>
</dialog>
