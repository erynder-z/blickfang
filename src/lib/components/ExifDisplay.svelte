<script lang="ts">
  ...
  $: exifData = (() => {
    if (!$imageExif) return null;
    try {
      return sortExifDataByTag(JSON.parse($imageExif));
    } catch (e) {
  ...
  /**
   * Sorts the EXIF data by tag name.
   * @param {Record<string, string>} exifData - The EXIF data to sort.
   * @returns {Record<string, string>} - The sorted EXIF data.
   */
  const sortExifDataByTag = (data: Record<string, string>) =>
